...
Поэтому я предпочту потратить побольше времени и написать свою нативную реализацию компилятора Sass. Это хорошо скажется и на производительности, и на весе бинарника, и даст хороший опыт понимания работы компиляторов.
...
- В папке `content` находятся страницы в формате Markdown, кроме того там же можно расположить статические файлы (например, изображения);
  - При сборке проверяются ссылки (например, если изображение используется на страницах, оно минифицируется и в готовый документ попадает ссылка на уменьшенный файл; если изображение нигде не используется, оно не попадает в итоговую сборку);
...
- Содержимое папки `static` копируется в выходную директорию (по умолчанию `public`) без изменений;
