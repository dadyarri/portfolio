...
Каждый контейнер, который должен быть доступен по своему поддомену, должен быть подключен к общей с веб-сервером сети (в моём случае это `caddy`). Тогда Docker автоматически определит IP-адрес контейнера по названию сервиса из Docker Compose. При этом ни одно приложение не светит портами наружу и по HTTP к ним нельзя подключиться напрямую.
